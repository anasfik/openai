...
void main() async {
  ...
  try {
    ...
    OpenAIConversationItemsResponse response =
        await OpenAI.instance.conversations.listItems(
      conversationId: conversationId,
    );

    print(
        "Found ${response.data.length} items in conversation $conversationId");
    ...
    OpenAIConversationItemsResponse paginatedResponse =
        await OpenAI.instance.conversations.listItems(
      conversationId: conversationId,
      limit: 10, // Limit to 10 items
      order: "desc", // Order by creation time descending
      after:
          response.lastId, // Start after the last item from previous response
    );

    print(
        "Found ${paginatedResponse.data.length} items (limit: 10, order: desc)");
